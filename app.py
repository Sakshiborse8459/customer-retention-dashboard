...
def load_data(path: str) -> pd.DataFrame:
    ...
    df = pd.read_csv("enhanced_customer_analysis.csv")
...
def customer_explorer_tab(filtered_df: pd.DataFrame) -> None:
    ...
    st.download_button(
        ...
        data=csv_data,
        file_name="\"enhanced_customer_analysis.csv",
        mime="text/csv",
...
def main() -> None:
    ...
    try:
        df = load_data("enhanced_customer_analysis.csv")
    except FileNotFoundError:
        st.error(
            f"Dataset file '{"enhanced_customer_analysis.csv"}' was not found. Place the CSV file in the same folder as app.py and rerun the dashboard."
        )
        st.stop()
